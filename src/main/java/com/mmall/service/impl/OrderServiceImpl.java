...
public class OrderServiceImpl implements IOrderService {
    ...
    public ServerResponse getOrderCartProduct(Integer userId) {
        ...
        for (OrderItem orderItem : orderItemList) {
            payment = BigDecimalUtil.mul(orderItem.getTotalPrice().doubleValue(), payment.doubleValue());
    ...
    public ServerResponse<OrderVo> manageDetail(Long orderNo) {
        ...
        if (order != null) {
            List<OrderItem> orderItemList = orderItemMapper.getOrderNo(orderNo);
            OrderVo orderVo = assembleOrderVo(order, orderItemList);
            return ServerResponse.createBySuccess(orderVo);
        }
    ...
    public ServerResponse<PageInfo> manageSearch(Long orderNo, @RequestParam(value = "pageNum", defaultValue = "1") int pageNum, @RequestParam(value = "pageSize", defaultValue = "10") int pageSize) {
        PageHelper.startPage(pageNum, pageSize);
        Order order = orderMapper.selectByOrderNo(orderNo);
        if (order != null) {
            List<OrderItem> orderItemList = orderItemMapper.getOrderNo(orderNo);
    ...
    private List<OrderVo> assembleOrderVoList(List<Order> orderList, Integer userId) {
        ...
        for (Order order : orderList) {
            ...
            if (userId == null) {
                // todo 管理员查询的时候，不需要穿userId
                orderItemList = orderItemMapper.getOrderNo(order.getOrderNo());
            } else {
    ...
    private ServerResponse getCartOrderItem(Integer userId, List<Cart> cartList) {
        ...
        for (Cart cartItem : cartList) {
            ...
            if (Const.ProductStatusEnum.ON_SALE.getCode() != product.getStatus()) {
                // 查询产品的状态，如果不是在线售卖状态的话，返回错误信息
                return ServerResponse.createByErrorMessage("产品" + product.getName() + "不是在线售卖状态");
            }

            if (cartItem.getQuantity() > product.getStock()) {
                //当cartItem需要的数量大于product的库存的时候，返回错误信息
...
//        Long orderNo = (Long)params.get("out_trade_no");   这样写会报错cannot cast 'java.lang.String' to 'java.lang.Long'
